...
#[cfg(feature = "policy_v2")]
use crate::mig_policy::{get_init_event_log, get_init_policy, get_init_tdreprot};
#[cfg(feature = "policy_v2")]
...
pub fn client_rebinding<T: AsyncRead + AsyncWrite + Unpin>(
    ...
    let certs = create_certificate_for_rebinding_old(&signing_key, binding_handle, target_td_uuid)
        .map_err(|e| {
            log::error!("server rebinding gen_cert() failed with error {:?}\n", e);
            e
        })?;
...
fn create_certificate_for_rebinding_old(
    ...
    let servtd_ext = read_servtd_ext(binding_handle, target_td_uuid)
        .map_err(|_| RatlsError::GenerateCertificate)?;
    let init_policy_hash =
        digest_sha384(get_init_policy(binding_handle).as_deref().unwrap_or(policy))
            .map_err(|_| RatlsError::GenerateCertificate)?;
    let init_tdreport = get_init_tdreprot(binding_handle)
        .or_else(|| Some(tdreport.as_bytes().to_vec()))
        .ok_or(RatlsError::GenerateCertificate)?;
    let init_event_log = get_init_event_log(binding_handle)
        .or_else(|| Some(event_log.to_vec()))
        .ok_or(RatlsError::GenerateCertificate)?;
