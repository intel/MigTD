...
pub struct MigtdMigrationInformation {
    ...
    // clears this untrusted field so policy v2 uses the authenticated
    // SERVTD_EXT hash instead.
    //
...
impl MigtdMigrationInformation {
    ...
    ///
    /// A correctly framed legacy flag and tail are accepted and normalized
    /// away so host-supplied TDINFO cannot influence policy decisions.
