    ...
    #[cfg(not(feature = "spdm_attestation"))]
    {
        ...
        if info.is_src() {
            ...
            .map_err(|_| {
                ...
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk(): Failed in ratls transport.\n");
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_client.write timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_client.write error: {:?}\n", e);
                e
            })?;
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_client.read timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_client.read error: {:?}\n", e);
                e
            })?;
            if size < size_of::<ExchangeInformation>() {
                ...
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk(): Incorrect ExchangeInformation size Migration ID: {}. Size - Expected: {} Actual: {}\n", info.mig_info.mig_request_id, size_of::<ExchangeInformation>(), size);
            ...
            ratls_client.transport_mut().shutdown().await.map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_client.transport_mut().shutdown() error: {:?}\n", e);
                e
            })?;
            ...
            ratls_client
                ...
                .map_err(|e| {
                    ...
                    log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: Failed to transport in vmcall_raw_instance with Migration ID: {} errorcode: {}", info.mig_info.mig_request_id, e);
                    MigrationResult::InvalidParameter
                })?;
        } else {
            // TLS server
            ...
            .map_err(|_| {
                ...
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk(): Failed in ratls transport. Migration ID: {:x}\n", info.mig_info.mig_request_id);
                MigrationResult::SecureSessionError
            })?;
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_server.write timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_server.write error: {:?}\n", e);
                e
            })?;
            let size = with_timeout(
                TLS_TIMEOUT,
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_server.read timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: ratls_server.read error: {:?}\n", e);
                e
            })?;
            if size < size_of::<ExchangeInformation>() {
                #[cfg(feature = "vmcall-raw")]
            ...
            ratls_server
                ...
                .map_err(|e| {
                    ...
                    log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: Failed to transport in vmcall_raw_instance with Migration ID: {} errorcode: {}\n", info.mig_info.mig_request_id, e);
                    MigrationResult::InvalidParameter
                })?;
        }
    ...
    #[cfg(feature = "spdm_attestation")]
    {
        ...
        if info.is_src() {
            let mut spdm_requester = spdm::spdm_requester(transport).map_err(|_e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk(): Failed in spdm_requester transport. Migration ID: {}\n", info.mig_info.mig_request_id);
                MigrationResult::SecureSessionError
            })?;
            with_timeout(
                SPDM_TIMEOUT,
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: spdm_requester_transfer_msk timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: spdm_requester_transfer_msk error: {:?}\n", e);
                e
            })?;
            ...
            let mut spdm_responder = spdm::spdm_responder(transport).map_err(|_e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk(): Failed in spdm_responder transport. Migration ID: {}\n", info.mig_info.mig_request_id);
                MigrationResult::SecureSessionError
            })?;
            ...
            .await
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: spdm_responder_transfer_msk timeout error: {:?}\n", e);
                e
            })?
            .map_err(|e| {
                log::error!(migration_request_id = info.mig_info.mig_request_id; "exchange_msk: spdm_responder_transfer_msk error: {:?}\n", e);
                e
            })?;
            log::info!(migration_request_id = info.mig_info.mig_request_id;"MSK exchange completed\n");
        }
