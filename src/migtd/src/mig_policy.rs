...
mod v2 {
    ...
    pub fn init_policy(
        ...
        if let Some(cose) = crate::config::get_servtd_corim() {
            ...
            verified_policy
                .verify_signer_chains_not_revoked(verified_policy.servtd_crl.as_bytes())?;
        }
    ...
    fn verify_policy_and_event_log<'p>(
        ...
        let local_servtd_crl = local_policy.servtd_crl.as_bytes();
        #[cfg_attr(not(feature = "servtd_corim"), allow(unused_mut))]
    ...
    fn verify_peer_signer_revocation(
        ...
        peer_policy
            .verify_signer_chains_not_revoked(local_policy.servtd_crl.as_bytes())
    ...
    /// Extract the CRL number from the policy being evaluated. Local CRL
    /// authority is applied separately when checking signer revocation.
    fn servtd_crl_num_from_policy(policy: &VerifiedPolicy) -> Result<u32, PolicyError> {
        get_crl_number(policy.servtd_crl.as_bytes()).map_err(|_| PolicyError::InvalidCollateral)
    }
    ...
    fn setup_evaluation_data(
        ...
        let migtd = policy
            .servtd_lookup_by_report(&report_value)
            .ok_or(PolicyError::UnqualifiedMigTdInfo)?;
        if policy.requires_servtd_tcb_status() && migtd.tcb_status.is_none() {
            return Err(PolicyError::UnqualifiedMigTdInfo);
        }
        ...
        Ok(PolicyEvaluationInfo {
            ...
            fmspc: Some(fmspc),
            migtd_isvsvn: Some(migtd.isvsvn),
            migtd_tcb_status: migtd.tcb_status,
            migtd_tcb_date: migtd.tcb_date,
            pck_crl_num: Some(pck_crl_num),
            root_ca_crl_num: Some(root_ca_crl_num),
            servtd_crl_num: Some(servtd_crl_num_from_policy(policy)?),
        })
    }
    ...
    #[cfg(test)]
    fn endorse_test_report(policy: &mut VerifiedPolicy, tdreport: &TdxReport) {
        policy.servtd_tcb_mapping.as_mut().unwrap().svn_mappings[0].td_measurements =
            Measurements::new_from_bytes(&tdinfo_hash_from_td_info(&tdreport.td_info).unwrap());
    }
        ...
        fn peer_crl_number_is_not_replaced_by_local_crl_number() {
            let policy_input = |crl: &[u8]| {
                ...
                let collateral = policy["policyData"]["servtdCollateral"]
                    .as_object_mut()
                    .unwrap();
                // Keep this CRL-number regression independent of optional identity lookup.
                collateral.remove("servtdIdentity");
                collateral.remove("servtdIdentityIssuerChain");
                serde_json::to_vec(&policy).unwrap()
            };
            let local_input = policy_input(fixtures::REVOKED_POLICY_CRL);
            let peer_input = policy_input(fixtures::EMPTY_CRL);
            let mut local = load_policy(&local_input, fixtures::ROTATED_POLICY_CHAIN);
            let mut peer = load_policy(&peer_input, fixtures::ROTATED_POLICY_CHAIN);
            verify_peer_signer_revocation(&peer, &local).unwrap();

            let tdreport =
                TdxReport::read_from_bytes(&[0; core::mem::size_of::<TdxReport>()]).unwrap();
            endorse_test_report(&mut peer, &tdreport);
            endorse_test_report(&mut local, &tdreport);
        ...
        fn peer_cannot_replace_local_revocations_with_its_own_crl() {
            ...
            crypto::validate_peer_cert_chain(
                local
                    ...
                    .unwrap()
                    .as_bytes(),
                peer.servtd_tcb_mapping_issuer_chain
                    .as_deref()
                    .unwrap()
                    .as_bytes(),
            )
        ...
        fn peer_identity_must_contain_the_local_crl_issuer() {
            ...
            crypto::validate_peer_cert_chain(
                local
                    ...
                    .unwrap()
                    .as_bytes(),
                peer.servtd_identity_issuer_chain
                    .as_deref()
                    .unwrap()
                    .as_bytes(),
            )
    ...
    mod evaluation_tests {
        ...
        const ISSUER_CHAIN: &[u8] =
            include_bytes!("../../policy/test/policy_v2/cert_chain/policy_issuer_chain.pem");

        fn policy_json() -> serde_json::Value {
            let mut value: serde_json::Value = serde_json::from_slice(include_bytes!(
                "../../policy/test/policy_v2/policy_v2.json"
            ))
            .unwrap();
            ...
            data.remove("forwardPolicy");
            data.remove("backwardPolicy");
            let collateral = data["servtdCollateral"].as_object_mut().unwrap();
            collateral.remove("servtdIdentity");
            collateral.remove("servtdIdentityIssuerChain");
            value
        }
        ...
        fn evaluation_results(
            policy: &VerifiedPolicy,
            tdreport: &TdxReport,
        ) -> [Result<PolicyEvaluationInfo, PolicyError>; 2] {
            let mut suppl_data = [0; REPORT_DATA_SIZE];
            let td_info_range = Report::R_MIGTD_ATTR_TD.start..Report::R_MIGTD_RTMR3.end;
            let td_info_size = td_info_range.len();
            suppl_data[td_info_range].copy_from_slice(&tdreport.td_info.as_bytes()[..td_info_size]);
            let status = TcbStatus::UpToDate.as_str().as_bytes();
            suppl_data[Report::R_TCB_STATUS.start..][..status.len()].copy_from_slice(status);
            assert_eq!(
                compute_tdinfo_hash_from_report(&Report::new(&suppl_data).unwrap()).unwrap(),
                tdinfo_hash_from_td_info(&tdreport.td_info).unwrap()
            );

            [
                setup_evaluation_data(
                    [0x00, 0xC0, 0x6F, 0x00, 0x00, 0x00],
                    &suppl_data,
                    policy,
                    policy.get_collaterals(),
                ),
                setup_evaluation_data_with_tdreport(tdreport, policy),
            ]
        }

        fn assert_unendorsed(policy: &VerifiedPolicy, tdreport: &TdxReport) {
            assert!(!policy.requires_servtd_tcb_status());
            let hash = tdinfo_hash_from_td_info(&tdreport.td_info).unwrap();
            assert!(policy.servtd_lookup_by_tdinfo_hash(&hash).is_none());

            let crl_only = PolicyEvaluationInfo {
                servtd_crl_num: Some(servtd_crl_num_from_policy(policy).unwrap()),
                ..Default::default()
            };
            for skip_global in [false, true] {
                policy
                    .policy_data
                    .evaluate_policy_common(&crl_only, &crl_only, skip_global)
                    .unwrap();
                policy
                    .policy_data
                    .evaluate_policy_forward(&crl_only, &crl_only, skip_global)
                    .unwrap();
            }

            let results = evaluation_results(policy, tdreport);
            assert!(
                results
                    .iter()
                    .all(|result| matches!(result, Err(PolicyError::UnqualifiedMigTdInfo))),
                "{results:?}"
            );
        }

        #[test]
        fn evaluation_requires_a_mapping_without_identity() {
            let mut value = policy_json();
            let data = value["policyData"].as_object_mut().unwrap();
            let collateral = data.remove("servtdCollateral").unwrap();
            data.insert("servtdCrl".into(), collateral["servtdCrl"].clone());
        ...
        #[test]
        fn evaluation_rejects_a_json_mapping_miss_without_identity() {
            let input = serde_json::to_vec(&policy_json()).unwrap();
            let policy = load_policy(&input, ISSUER_CHAIN);
            let tdreport =
                TdxReport::read_from_bytes(&[0; core::mem::size_of::<TdxReport>()]).unwrap();
            assert_unendorsed(&policy, &tdreport);
        ...
        #[test]
        fn evaluation_accepts_an_endorsed_peer_without_identity() {
            let input = serde_json::to_vec(&policy_json()).unwrap();
            let mut policy = load_policy(&input, ISSUER_CHAIN);
            let tdreport =
                TdxReport::read_from_bytes(&[0; core::mem::size_of::<TdxReport>()]).unwrap();
            endorse_test_report(&mut policy, &tdreport);
    ...
    #[cfg(test)]
    fn continuity_policy() -> VerifiedPolicy<'static> {
        let input = include_bytes!("../../policy/test/policy_v2/policy_v2.json");
        let chain =
            include_bytes!("../../policy/test/policy_v2/cert_chain/policy_issuer_chain.pem");
        let mut policy = RawPolicyData::deserialize_from_json(input)
            .unwrap()
            .verify(chain)
            .unwrap();
        policy.servtd_identity = None;
        policy.servtd_identity_issuer_chain = None;
        policy.servtd_tcb_mapping = Some(TdTcbMapping {
            id: String::from("TEST"),
            version: 1,
    ...
    #[cfg(test)]
    fn verify_continuity_order(
        policy: &VerifiedPolicy,
        init_hash: &[u8; SHA384_DIGEST_SIZE],
        init_attr: &[u8; 8],
        cur_hash: &[u8; SHA384_DIGEST_SIZE],
        cur_attr: &[u8; 8],
    ) -> Result<(), PolicyError> {
        let mut servtd_ext =
            ServtdExt::read_from_bytes(&[0; core::mem::size_of::<ServtdExt>()]).unwrap();
        servtd_ext.init_servtd_info_hash = *init_hash;
        servtd_ext.init_attr = *init_attr;
        servtd_ext.cur_servtd_info_hash = *cur_hash;
        servtd_ext.cur_servtd_attr = *cur_attr;
        let evaluation = PolicyEvaluationInfo {
            migtd_isvsvn: policy
                .servtd_lookup_by_tdinfo_hash(cur_hash)
    ...
    fn init_servtd_svn_lookup_misses_fail_closed() {
        let mapping = continuity_policy();
        assert!(matches!(
            verify_continuity_order(&mapping, &[0x33; 48], &[0; 8], &[0x22; 48], &[0; 8],),
            Err(PolicyError::UnqualifiedMigTdInfo)
        ));
        assert!(matches!(
            verify_continuity_order(&mapping, &[0x11; 48], &[0; 8], &[0x33; 48], &[0; 8],),
            Err(PolicyError::UnqualifiedMigTdInfo)
        ));
    }
    ...
    fn source_mapping_can_authorize_release_unknown_to_destination() {
        let source_mapping = continuity_policy();
        let mut destination_mapping = continuity_policy();
        destination_mapping.servtd_tcb_mapping = Some(TdTcbMapping {
        ...
        assert!(verify_continuity_order(
            &source_mapping,
            &[0x11; 48],
            &[0; 8],
            &[0x22; 48],
