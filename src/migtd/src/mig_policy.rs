...
mod v2 {
    ...
    pub fn authenticate_remote(
        ...
        let policy_signer = get_policy_issuer_chain().ok_or(PolicyError::InvalidParameter)?;
        if is_src {
            authenticate_migration_dest(quote_peer, event_log_peer, policy_peer, policy_signer)
        } else {
            authenticate_migration_source(quote_peer, event_log_peer, policy_peer, policy_signer)
        }
    }
    ...
    fn authenticate_migration_dest(
        ...
        let (evaluation_data_dst, verified_policy_dst, suppl_data) =
            authenticate_remote_common(quote_dst, event_log_dst, mig_policy_dst, policy_signer)?;
    ...
    fn authenticate_migration_source(
        ...
        let (evaluation_data_src, verified_policy_src, suppl_data) =
            authenticate_remote_common(quote_src, event_log_src, mig_policy_src, policy_signer)?;
        let relative_reference = get_local_tcb_evaluation_info()?;
        let policy = get_verified_policy().ok_or(PolicyError::InvalidParameter)?;
    ...
    fn verify_quote(
        ...
        let fmspc = get_fmspc_from_quote(&quote)?;
        let suppl_data = include_bytes!("../../../suppl_data.dat").to_vec();
    ...
    fn setup_evaluation_data(
        ...
        let migtd_svn = policy.servtd_tcb_mapping.get_migtd_svn(&report_value);
    ...
    fn test_authenticate() {
        let quote = include_bytes!("../../../quote.dat");
        let policy = include_bytes!("../../../policy_v2_signed.json");
        let raw_policy = RawPolicyData::deserialize_from_json(policy).unwrap();
        let cert_chain = include_bytes!("../../../cert_chain/migtd_issuer_chain.pem");
        let verified_policy = raw_policy.verify(cert_chain, None, None).unwrap();
        LOCAL_TCB_INFO
            .try_call_once(|| {
                let (fmspc, suppl_data) = verify_quote(quote, verified_policy.get_collaterals())?;
                setup_evaluation_data(
                    fmspc,
                    &suppl_data,
                    &verified_policy,
                    verified_policy.get_collaterals(),
                )
            })
            .map(|_| ())
            .unwrap();
        let cert_chain = include_bytes!("../../../cert_chain/migtd_issuer_chain.pem");
        init_policy(policy, cert_chain).unwrap();
        authenticate_migration_dest(quote, &[], policy, cert_chain).unwrap();
    }
}
