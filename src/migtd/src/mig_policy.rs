...
mod v2 {
    ...
    struct InitMigtdData {
        init_td_report: Vec<u8>,
        init_event_log: Vec<u8>,
        init_mig_policy: Vec<u8>,
    }
    ...
    lazy_static! {
        ...
        static ref INIT_MIGTD_DATA: Mutex<BTreeMap<u64, InitMigtdData>> =
    ...
    pub fn put_init_migtd_data(
        binding_handle: u64,
        init_policy: &[u8],
        init_event_log: &[u8],
        init_td_report: &[u8],
    ) {
        let data = InitMigtdData {
            init_event_log: init_event_log.to_vec(),
            init_td_report: init_td_report.to_vec(),
            init_mig_policy: init_policy.to_vec(),
        };
        INIT_MIGTD_DATA.lock().insert(binding_handle, data);
    }

    pub fn get_init_policy(binding_handle: u64) -> Option<Vec<u8>> {
        INIT_MIGTD_DATA
            .lock()
            .get(&binding_handle)
            .map(|init| init.init_mig_policy.to_vec())
    }

    pub fn get_init_tdreprot(binding_handle: u64) -> Option<Vec<u8>> {
        INIT_MIGTD_DATA
            .lock()
            .get(&binding_handle)
            .map(|init| init.init_td_report.to_vec())
    }

    pub fn get_init_event_log(binding_handle: u64) -> Option<Vec<u8>> {
        INIT_MIGTD_DATA
            .lock()
            .get(&binding_handle)
            .map(|init| init.init_event_log.to_vec())
    }

    fn remove_init_migtd_data(binding_handle: u64) -> Option<InitMigtdData> {
        INIT_MIGTD_DATA.lock().remove(&binding_handle)
    }
