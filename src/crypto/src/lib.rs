...
/// in addition to the RTMR1 signer-anchor binding, which cannot distinguish an
/// unrevoked certificate from a revoked one under the same root, leaf Subject
/// DN/SAN, and signer EKU.
/// Freshness/anti-rollback (monotonic CRL number) is enforced by the policy
/// layer, not here.
