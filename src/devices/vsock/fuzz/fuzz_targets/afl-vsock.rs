...
use fuzzlib::{dma_init, COMMON_HEADER, DMA_ALLOCATOR};
...
impl VsockDmaPageAllocator for Allocator {
    fn alloc_pages(&self, page_num: usize) -> Option<u64> {
        DMA_ALLOCATOR.lock().get_mut()?.alloc_pages(page_num)
    }
...
fn fuzz_vsock(paddr: u64, packet: &[u8])  {
    ...
    // server_socket.bind(&listen_addrss).unwrap();
...
fn main() {
    let tmp = [
        ...
        0x0c, 0x00, 0x00, 0xfe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf4, 0x1a, 0x00, 0x11, 
    ...
    dma_init(paddr, TD_PAYLOAD_DMA_SIZE, 1);
