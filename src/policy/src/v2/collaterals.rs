...
pub type Collaterals<'a> = BTreeMap<String, Collateral<'a>>;

/// Deserialize Collaterals from JSON byte slice
pub fn parse_collaterals_from_bytes(json: &[u8]) -> Result<Collaterals, PolicyError> {
...
pub struct Collateral<'a> {
    ...
    pub tee_type: u32,
    pub pck_crl_issuer_chain: &'a str,
    pub root_ca_crl: &'a str,
    pub pck_crl: &'a str,
    pub tcb_info_issuer_chain: &'a str,
    pub qe_identity_issuer_chain: &'a str,
...
impl<'a> Collateral<'a> {
    ...
    pub fn from_json_slice(json: &'a [u8]) -> Result<Self, PolicyError> {
        serde_json::from_slice(json).map_err(|_| PolicyError::InvalidCollateral)
    }
}
...
pub fn get_collateral_with_fmspc<'a>(
    fmspc: &[u8],
    collaterals: &'a Collaterals<'a>,
) -> Result<&'a Collateral<'a>, PolicyError> {
    if fmspc.len() != 6 {
...
mod test {
    ...
    #[test]
    fn test_parse_collaterals_from_bytes() {
        let collaterals_json = include_bytes!("../../test/policy_v2/collaterals.json");
        let result = parse_collaterals_from_bytes(collaterals_json);
        assert!(result.is_ok());
    ...
    fn test_get_collateral_with_fmspc() {
        let fmspc = [0x10, 0xA0, 0x6D, 0x07, 0x00, 0x00];
        let collaterals_bytes = include_bytes!("../../test/policy_v2/collaterals.json");
        let collaterals = super::parse_collaterals_from_bytes(collaterals_bytes).unwrap();
    ...
    fn test_get_tcb_evaluation_number_from_collateral() {
        let collaterals_json = include_bytes!("../../test/policy_v2/collaterals.json");
        let collaterals = parse_collaterals_from_bytes(collaterals_json).unwrap();

        let collateral = collaterals.get("20A06D080000").unwrap();
        let tcb_evaluation_number = get_tcb_evaluation_number_from_collateral(collateral);

        assert!(tcb_evaluation_number.is_ok());
        assert_eq!(tcb_evaluation_number.unwrap(), 17);
    }
}
