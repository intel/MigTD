...
pub struct VerifiedServtdCollateral {
    ...
    pub servtd_tcb_mapping: ServtdSvnMapping,
}
...
pub struct ServtdCollateral<'a> {
    ...
    pub servtd_tcb_mapping: RawServtdSvnMapping<'a>,
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RawServtdSvnMapping<'a> {
    #[serde(borrow)]
    pub td_tcb_mapping: &'a RawValue,
    pub signature: String,
}

impl<'a> RawServtdSvnMapping<'a> {
    pub fn deserialize_from_json(slice: &'a [u8]) -> Result<Self, PolicyError> {
        serde_json::from_slice::<RawServtdSvnMapping>(slice)
            .map_err(|_| PolicyError::InvalidEngineSvnMap)
    }

    pub fn verify_signature(&self, issuer_chain: &[u8]) -> Result<ServtdSvnMapping, PolicyError> {
        let signature = hex_string_to_bytes(&self.signature)?;
        ...
        serde_json::from_str::<ServtdSvnMapping>(self.td_tcb_mapping.get())
            .map_err(|_| PolicyError::InvalidPolicy)
    }
}
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ServtdSvnMapping {
    pub td_tcb_mapping: TcbMapping,
}

impl ServtdSvnMapping {
    pub fn deserialize_from_json(json: &[u8]) -> Result<Self, PolicyError> {
        serde_json::from_slice::<ServtdSvnMapping>(json).map_err(|_| PolicyError::InvalidPolicy)
    }

    pub fn get_migtd_svn(&self, report: &Report) -> Option<u16> {
        self.td_tcb_mapping.get_engine_svn_by_report(report)
    }
}

#[derive(Debug, Serialize, Deserialize)]
pub struct TcbMapping {
    pub id: String,
    pub version: u32,
    pub issue_date: String,
    pub next_update: String,
...
impl TcbMapping {
    ...
    pub fn get_engine_svn_by_measurements(&self, measurements: &Measurements) -> Option<u16> {
        for mapping in &self.svn_mappings {
            if &mapping.td_measurements == measurements {
...
mod test {
    ...
    fn test_get_engine_svn() {
        ...
        let engine: TcbMapping = serde_json::from_slice(engine_bytes).unwrap();

        let td_measurements = Measurements {
            mrtd: String::from("01234567890abcdef1234567890abcdef1234567890abcdef1234567890abcde"),
            rtmr0: String::from("bcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890a"),
            rtmr1: String::from("fedcba0987654321fedcba0987654321fedcba0987654321fedcba0987654321"),
        };
        assert_eq!(
        ...
        let td_measurements = Measurements {
            mrtd: String::from("01234567890abcdef1234567890abcdef1234567890abcdef1234567890abcde"),
            rtmr0: String::from("abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890"),
            rtmr1: String::from("fedcba0987654321fedcba0987654321fedcba0987654321fedcba0987654321"),
        };
        assert!(engine
