...
use crate::{
    ...
    Collaterals, EventName, PolicyError, ServtdCollateral, ServtdSvnMapping, TdIdentity,
};
...
mod test {
    ...
    use crypto::ecdsa::pem_to_der_from_slice;

    #[test]
    fn test_parse_raw_policy_data() {
        let policy = include_str!("../../test/policy_v2/policy_data.json");
        let policy_data = serde_json::from_str::<RawPolicyData>(policy).unwrap();
        assert!(policy_data.get_collaterals().is_ok());
    }

    #[test]
    fn test_verify_policy() {
        let policy_data = include_bytes!("../../test/policy_v2/policy_signed.json");
        let policy = RawPolicyData::deserialize_from_json(policy_data).unwrap();
        let issuer_chain = include_bytes!("../../test/policy_v2/cert_chain/migtd_issuer_chain.pem");
    ...
    fn test_sign_policy() {
        extern crate std;
        let policy_data = include_bytes!("../../../../policy_v2_pre_production.json");

        let pkcs8 =
            pem_to_der_from_slice(include_bytes!("../../../../cert_chain/issuer_pkcs8.key"))
                .unwrap();

        let signature = crypto::ecdsa::ecdsa_sign(&pkcs8, policy_data).unwrap();
        let policy = RawPolicyData {
            policy_data: serde_json::from_slice::<&RawValue>(policy_data).unwrap(),
