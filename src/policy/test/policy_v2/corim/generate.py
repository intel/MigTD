...
def signed_corim(payload, leaf, root, key):
...
def generate(policy_path):
    ...
    leaf = certificate(
        leaf_name, root_name, leaf_key.public_key(), root_key, 2, False, san_names=san_names,
    )
    chain_pem = (
    ...
    (output / "tcb_mapping.corim").write_bytes(signed_corim(payload, leaf, root, leaf_key))
    ...
    for serial, (name, subject, names, purposes) in enumerate(variants, start=3):
        key = ec.generate_private_key(ec.SECP384R1())
        cert = certificate(
            subject, root_name, key.public_key(), root_key, serial, False,
            san_names=names, eku_oids=purposes,
        )
        (output / f"tcb_mapping_{name}.corim").write_bytes(signed_corim(payload, cert, root, key))
