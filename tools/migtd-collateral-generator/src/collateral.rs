...
impl Collateral {
    ...
    pub fn new(
        ...
        // Convert root_ca_crl from DER to PEM format if needed
        let root_ca_crl = if root_ca_crl.starts_with(b"-----BEGIN") {
            // Already in PEM format, just convert to String
            String::from_utf8(root_ca_crl)
                .map_err(|e| anyhow!("Invalid UTF-8 in root CA CRL: {}", e))?
        } else {
            // Assume DER format, convert to PEM
            der_to_pem(&root_ca_crl, "X509 CRL")
        };
